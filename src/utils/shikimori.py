...
from src.schemas.parsers import ParsedTitle
...
class Shikimori:
    ...
    async def get_title(self, title: ParsedTitle):
        start_time = datetime.now()
        title_info = api.animes.GET(
            search=title.en_name or title.name, kind=title.kind)
        end_time = datetime.now()
        if len(title_info) > 0:
            await self.service.set_shikimori_title(title_info[0]['id'], title_info[0])
            return title_info[0]
    ...
    async def get_shikimori_title(self, title_id: int, background_tasks: BackgroundTasks):
