...
from src.schemas.parsers import Genre, ParsedGenre, ParsedTitleShort, Title
...
class Parser:
    ...
    async def get_genres(self, background_tasks: BackgroundTasks, db: AsyncSession, service: ParserInfoService = Depends(Provide[Container.service])) -> List[Genre]:
        ...
        return await self._prepare_genres(genres_objs, db)
        ...
        for parsed_title in titles:
            if parsed_title.id_on_website not in existing_ids_set:
                ...
                if await self.title_data_changed(parsed_title, title):
                    ...
                    background_tasks.add_task(self.update_title_in_db, title_data=parsed_title, db=db, title_id=title.id)
    ...
    async def _prepare_genres(self, genres: List[ParsedGenre], existing_genres: List[GenreModel], db: AsyncSession) -> List[Genre]:
        ...
        for parsed_genre in genres:
            if parsed_genre.id_on_website not in existing_ids_set:
                ...
                genre = next(genre for genre in existing_genres if genre.id_on_website == parsed_genre.id_on_website)
            
            db_genres.append(genre)
        
        return db_genres

    async def get_genre(self, genre_id: UUID, page: int, background_tasks: BackgroundTasks, db: AsyncSession, service: ParserInfoService = Depends(Provide[Container.service])):
